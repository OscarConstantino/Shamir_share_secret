...
class Shamir:
    ...
    def __init__(self,secret, n_parts, k_parts):
    ...
    def lagrange_basis_calculation(self, x_shares, j):
        lagrange_base = 1
        x_j = x_shares[j]
        for m, x_m in enumerate(x_shares):
            if m != j:
                numerator = (0 - x_m) % Shamir.prime_number
                denominator = (x_j - x_m) % Shamir.prime_number
                lagrange_base = (lagrange_base * numerator * Shamir.modular_inverse(self, denominator, Shamir.prime_number)) % Shamir.prime_number
        return lagrange_base

    def reconstruct_secret_shamir(self, min_points):
        print("POINTS USED: ", min_points)
        x_shares, y_shares = zip(*min_points)
        secret = 0
        for j in range(len(min_points)):
            l_b = Shamir.lagrange_basis_calculation(self, x_shares, j)
            secret = (secret + y_shares[j] * l_b) % Shamir.prime_number
        return secret